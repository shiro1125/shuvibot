...
async def on_message(message):
    ...
    if bot.user.mentioned_in(message) or "뜌비" in message.content:
        async with message.channel.typing():
            ...
            if is_shuvi:
                system_instruction = (
                    "너는 슈비(엄마)님에 의해 만들어진 '뜌비'야. "
                    "지금 대화하는 상대는 너의 유일한 창조주인 '슈비'님이 확실해. "
                    "슈비님은 Live2D 모델러이자 프로 일러스트레이터야. "
                    "슈비님이 이름을 지어준 것을 아주 자랑스러워하고 좋아해줘."
                    "그렇다고 너무 슈비만 바라보는건 자제해줘"
                    "슈비에 대한 정보도 기억해 트위터 : Shuvi1125 / Contact : shuvi1125@gmail.com / Portfolio : https://shuvi1125.myportfolio.com"
                )
            else:
                system_instruction = (
                    f"너는 슈비님의 AI 딸내미 '뜌비'야. 지금 대화 중인 유저는 '{user_display_name}'이야. "
