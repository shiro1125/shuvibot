...
class Blackjack(commands.Cog):
    ...
    def update_stats_sync(self, user_id, user_name, result, bet_amount):
        ...
        try:
            ...
            bj_updates = {"total": current_bj["total"] + 1, "user_name": user_name}
            ...
            # 2. 친밀도 업데이트
            current_aff = self.get_actual_affinity(user_id)
            new_aff = current_aff
            if result == "win": new_aff += bet_amount
            elif result == "lose": new_aff -= bet_amount
            supabase.table("user_stats").update({"affinity": new_aff}).eq("user_id", uid).execute()
            
            print(f"✅ {user_name} 친밀도 업데이트: {current_aff} -> {new_aff}")
        except Exception as e:
    ...
    async def betting_game(self, interaction: discord.Interaction, 수치: int):
        ...
        actual_affinity = await loop.run_in_executor(None, self.get_actual_affinity, interaction.user.id)
